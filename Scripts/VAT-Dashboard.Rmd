...
dat_te<-effective_vat_rates_bu%>%
 ...
 dat<-left_join(dat_r,dat_te,by = c("PRODUCT_INDUSTRY_CODE"))%>%
  ...
  dat$diff_effective_rate<-round(dat$diff_effective_rate,2)
...
plt <- plot_ly(vat_effective_final) %>% 
  add_trace(x = ~deciles, y = ~effective_vat_before_reform, type = 'bar', name = 'Before reform') %>% 
  add_trace(x = ~deciles, y = ~effective_vat_after_reform, type = 'bar', name = 'After reform') %>% 
    layout(
